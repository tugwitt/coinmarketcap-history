...
def download_data(currency, start_date, end_date, proxy):
  """
  ...
  try:
    page = r.get(url,timeout=5, proxies={'http': proxy, 'https': proxy})
...
def get_htmls(cryptocurrencies, start_date, end_date, proxy_path):

    from tqdm import tqdm

    pl.load_file(proxy_path)
    ...
    for crypto in cryptocurrencies:
        for i in range(5):
            try:
                out = download_data(crypto, start_date, end_date, pl.random().address())
            except:
                out = None
